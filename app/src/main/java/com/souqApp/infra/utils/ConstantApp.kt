package com.souqApp.infra.utils

const val APP_TAG = "app_tag"

const val LOGIN_AFTER_RESET_PASSWORD ="login_after_reset_password"

const val ID_ORDER = "id_order"
const val ID_PRODUCT = "id_products"
const val ID_ADDRESS = "id_address"
const val LOCATION_USER = "location_user"
const val PRODUCTS_TYPE = "Products_type"
const val ADDRESS_DETAILS = "address_details"
const val ID_SUBCATEGORY = "id_sub_category"
const val PHONE_NUMBER = "phone_number"
const val RESET_TOKEN = "reset_token"

const val PRODUCT_BY_SUB_CATEGORY = 0
const val ALL_PRODUCTS = 1
const val RECOMMENDED_PRODUCTS = 2

const val CONNECT_TIMEOUT: Long = 30
const val READ_TIMEOUT: Long = 30
const val WRITE_TIMEOUT: Long = 30


//firebase config
const val IS_PURCHASE_ENABLED = "Is_Purchase_Enabled"
const val SHOW_COPYRIGHTS = "Show_SouqApp_Copyrights"
const val ORDER_HISTORY_ANDROID = "Order_history_Android"
const val MIN_ANDROID_VERSION = "Min_Android_Version"


const val BEST_MATCH = 0
const val NEW_ARRIVAL = 1
const val BEST_SELLING = 2
const val A_TO_Z = 3
const val Z_TO_A = 4
const val PRICE_LOW_TO_HIGH = 5
const val PRICE_HIGH_TO_LOW = 6




