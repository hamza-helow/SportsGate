...
class ChangeDefaultAddressUseCase @Inject constructor(private val addressRepository: AddressRepository) {

    suspend fun invoke(addressId: Int): Flow<Boolean> {
        return flow {
            val response = addressRepository.changeDefault(addressId)
